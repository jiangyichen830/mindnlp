...
def multi_scale_deformable_attention(
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        # -> batch_size*num_heads, num_queries, num_points, 2
        sampling_grid_l_ = (
            sampling_grids[:, :, :, level_id].swapaxes(1, 2).flatten(0, 1)
        )
...
class DetaMultiscaleDeformableAttention(nn.Cell):
    ...
    def __init__(self, config: DetaConfig, num_heads: int, n_points: int):
        ...
        self.attention_weights = nn.Dense(
...
class DetaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, DetaLearnedPositionEmbedding):
            ...
            module.column_embeddings.set_data(
                initializer(
                    Uniform(),
                    module.column_embeddings.shape,
                    module.column_embeddings.dtype,
                )
            )
    ...
    def get_reference_points(spatial_shapes, valid_ratios):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ref_y, ref_x = meshgrid(
                ops.linspace(0.5, height - 0.5, height, dtype=ms.float32),
                ops.linspace(0.5, width - 0.5, width, dtype=ms.float32),
                indexing="ij",
            )
...
class DetaModel(DetaPreTrainedModel):
        ...
        if config.num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list.append(
                    nn.SequentialCell(
                        nn.Conv2d(
                            in_channels, config.d_model, kernel_size=1, has_bias=True
                        ),
    ...
    def get_proposal_pos_embed(self, proposals):
        ...
        dim_t = ops.arange(num_pos_feats, dtype=ms.int64).float()
        dim_t = temperature ** (
    ...
    def gen_encoder_output_proposals(self, enc_output, padding_mask, spatial_shapes):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            grid_y, grid_x = meshgrid(
                ops.linspace(0, height - 1, height, dtype=ms.float32),
                ops.linspace(0, width - 1, width, dtype=ms.float32),
                indexing="ij",
            )
    ...
    def construct(
        ...
        if self.config.num_feature_levels > len(sources):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                pos_l = self.backbone.position_embedding(source, mask).astype(
                    source.dtype
                )
...
class DetaForObjectDetection(DetaPreTrainedModel):
    ...
    def __init__(self, config: DetaConfig):
        ...
        self.class_embed.bias.data = ops.ones(config.num_labels) * bias_value
        ...
        if config.with_box_refine:
            ...
            constant_init = ms.common.initializer.Constant(value=-2.0)
            self.bbox_embed[0].layers[-1].bias.data[2:] = constant_init(
                self.bbox_embed[0].layers[-1].bias.data[2:]
            )
            ...
            constant_init = ms.common.initializer.Constant(value=-2.0)
            self.bbox_embed.layers[-1].bias.data[2:] = constant_init(
                self.bbox_embed.layers[-1].bias.data[2:]
            )
        ...
        if config.two_stage:
            ...
            for box_embed in self.bbox_embed:
                box_embed.layers[-1].bias.data[2:].set_data(
                    initializer(
                        "zeros",
                        box_embed.layers[-1].bias.shape,
                        box_embed.layers[-1].bias.dtype,
                    )
                )
