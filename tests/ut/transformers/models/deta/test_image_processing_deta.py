...
class DetaImageProcessingTest(
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        expected_slice = ms.Tensor([0.2796, 0.3138, 0.3481])
        self.assertTrue(
            np.allclose(
        ...
        # verify area
        expected_area = ms.Tensor(
            [5887.9600, 11250.2061, 489353.8438, 837122.7500, 147967.5156, 165732.3438]
        )
        self.assertTrue(
            np.allclose(encoding["labels"][0]["area"], expected_area.asnumpy())
        )
        # verify boxes
        expected_boxes_shape = (6, 4)
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
        expected_boxes_slice = ms.Tensor([0.5503, 0.2765, 0.0604, 0.2215])
        self.assertTrue(
            np.allclose(
        ...
        self.assertTrue(
            np.allclose(encoding["labels"][0]["image_id"], expected_image_id.asnumpy())
        )
        # verify is_crowd
        expected_is_crowd = ms.Tensor([0, 0, 0, 0, 0, 0])
        self.assertTrue(
            np.allclose(encoding["labels"][0]["iscrowd"], expected_is_crowd.asnumpy())
        )
        # verify class_labels
        expected_class_labels = ms.Tensor([75, 75, 63, 65, 17, 17])
        self.assertTrue(
            np.allclose(
                encoding["labels"][0]["class_labels"],
                expected_class_labels.asnumpy(),
            )
        )
        # verify orig_size
        expected_orig_size = ms.Tensor([480, 640])
        self.assertTrue(
            np.allclose(
                encoding["labels"][0]["orig_size"],
        ...
        self.assertTrue(
            np.allclose(encoding["labels"][0]["size"], expected_size.asnumpy())
        )
    ...
    def test_call_pytorch_with_coco_panoptic_annotations(self):
        ...
        expected_boxes_shape = [6, 4]
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
        expected_boxes_slice = ms.Tensor([0.2625, 0.5437, 0.4688, 0.8625])
        self.assertTrue(
            np.allclose(
        ...
        # verify class_labels
        expected_class_labels = ms.Tensor([17, 17, 63, 75, 75, 93])
        self.assertTrue(
            np.allclose(
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        encoding = image_processing(
            ...
            return_tensors="np",  # do_convert_annotations=True
        )
    ...
    def test_max_width_max_height_resizing_and_pad_strategy(self):
        image_1 = ops.ones([200, 100, 3], dtype=ms.uint8)
        ...
        ### Check for batch
        image_2 = ops.ones([100, 150, 3], dtype=ms.uint8)
        ...
        inputs = image_processor(images=[image_1, image_2], return_tensors="np")
