...
if is_mindspore_available():
    ...
    from mindnlp.transformers import NatBackbone, NatForImageClassification, NatModel
...
class NatModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {
            "image-feature-extraction": NatModel,
            "image-classification": NatForImageClassification,
        }
        if is_mindspore_available()
        else {}
    )
...
class NatModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        return (
            AutoImageProcessor.from_pretrained("shi-labs/nat-mini-in1k-224")
            if is_vision_available()
            else None
        )
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = ms.tensor([0.3805, -0.8676, -0.3912])
        self.assertTrue(
            np.allclose(
